...
(
    ...
    initial begin
        sin_lut[ 0] = 8'd128; sin_lut[ 1] = 8'd140; sin_lut[ 2] = 8'd152; sin_lut[ 3] = 8'd164;
        sin_lut[ 4] = 8'd176; sin_lut[ 5] = 8'd187; sin_lut[ 6] = 8'd198; sin_lut[ 7] = 8'd208;
        sin_lut[ 8] = 8'd217; sin_lut[ 9] = 8'd226; sin_lut[10] = 8'd233; sin_lut[11] = 8'd240;
        ...
        sin_lut[20] = 8'd245; sin_lut[21] = 8'd240; sin_lut[22] = 8'd233; sin_lut[23] = 8'd226;
        sin_lut[24] = 8'd217; sin_lut[25] = 8'd208; sin_lut[26] = 8'd198; sin_lut[27] = 8'd187;
        sin_lut[28] = 8'd176; sin_lut[29] = 8'd164; sin_lut[30] = 8'd152; sin_lut[31] = 8'd140;
        sin_lut[32] = 8'd128; sin_lut[33] = 8'd115; sin_lut[34] = 8'd103; sin_lut[35] = 8'd91;
        sin_lut[36] = 8'd79;  sin_lut[37] = 8'd68;  sin_lut[38] = 8'd57;  sin_lut[39] = 8'd47;
        sin_lut[40] = 8'd38;  sin_lut[41] = 8'd29;  sin_lut[42] = 8'd22;  sin_lut[43] = 8'd15;
        sin_lut[44] = 8'd10;  sin_lut[45] = 8'd6;   sin_lut[46] = 8'd3;   sin_lut[47] = 8'd1;
        sin_lut[48] = 8'd1;   sin_lut[49] = 8'd1;   sin_lut[50] = 8'd3;   sin_lut[51] = 8'd6;
        sin_lut[52] = 8'd10;  sin_lut[53] = 8'd15;  sin_lut[54] = 8'd22;  sin_lut[55] = 8'd29;
        sin_lut[56] = 8'd38;  sin_lut[57] = 8'd47;  sin_lut[58] = 8'd57;  sin_lut[59] = 8'd68;
        sin_lut[60] = 8'd79;  sin_lut[61] = 8'd91;  sin_lut[62] = 8'd103; sin_lut[63] = 8'd115;
    end

    reg [23:0] rot_phase;
    reg [27:0] speed_mod_phase;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) speed_mod_phase <= 0;
        else speed_mod_phase <= speed_mod_phase + 1'b1;
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) rot_phase <= 0;
        else if (rot_phase >= ROT_MAX - 1) rot_phase <= 0;
        else rot_phase <= rot_phase + 1'b1 +
            ((speed_mod_raw > 8'd160) ? 2'd2 : (speed_mod_raw > 8'd140) ? 2'd1 : (speed_mod_raw < 8'd96) ? 2'd0 : 2'd1);
    end

    wire rot_tick = (rot_phase == 0);
    wire [5:0] sin_idx = rot_phase[23:18];
    wire [5:0] cos_idx = sin_idx + 6'd16;
    ...
    wire [5:0] zoom_idx = speed_mod_phase[27:22];
    wire [7:0] zoom_raw = sin_lut[zoom_idx];
    wire [7:0] zoom_factor = 8'd128 + ((zoom_raw - 8'd128) >> 2);
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            hold_ch1 <= 8'h80; hold_ch2 <= 8'h80; ch1_new <= 1'b0; ch2_new <= 1'b0; sample_valid <= 1'b0;
        end else begin
            sample_valid <= 1'b0;
            ...
            if ((ch1_valid && ch2_new) || (ch2_valid && ch1_new) || (ch1_valid && ch2_valid)) begin
                sample_valid <= 1'b1; ch1_new <= 1'b0; ch2_new <= 1'b0;
            end
        end
    end
    ...
    wire signed [9:0] rot_dx = rd_x[17:7];
    ...
    wire [2:0] color_sector = rot_phase[23:21];
    function [11:0] wheel_color;
        input [2:0] sector;
        begin
            case (sector)
                3'd0: wheel_color = {4'hF, 4'h0, 4'h0};
                3'd1: wheel_color = {4'hF, 4'h8, 4'h0};
                3'd2: wheel_color = {4'hF, 4'hF, 4'h0};
                3'd3: wheel_color = {4'h0, 4'hF, 4'h0};
                3'd4: wheel_color = {4'h0, 4'hF, 4'hF};
                3'd5: wheel_color = {4'h0, 4'h4, 4'hF};
                3'd6: wheel_color = {4'h8, 4'h0, 4'hF};
                3'd7: wheel_color = {4'hF, 4'h0, 4'h8};
            endcase
        end
    endfunction
    wire [11:0] dot_color = wheel_color(color_sector);
    ...
    wire any_hit = |hit;
    wire [3:0] hit_count = hit[0] + hit[1] + hit[2] + hit[3] + hit[4] + hit[5] + hit[6] + hit[7] + hit[8] + hit[9] + hit[10] + hit[11];
        ...
        if (de) begin
            ...
            if (any_hit) begin
                if (hit_count >= 4'd4) begin
                    ...
                    {vga_r, vga_g, vga_b} = {dot_color[11:8] >> 1, dot_color[7:4] >> 1, dot_color[3:0] >> 1};
                end
            end
            if (rot_tick) begin
