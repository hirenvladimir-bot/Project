...
module Oscilloscope_Acq (
    ...
    reg [6:0] xadc_daddr;
    reg       xadc_den;
    reg       channel_sel;   // latched at EOC: pre-toggle mux, identifies CH1/CH2
    ...
    //
    // Key fix: adc_mux_sel toggles at EOC (not DRDY), so the external mux
    // switches BEFORE the next conversion's sampling instant. channel_sel
    // captures the pre-toggle value at EOC — it identifies which channel
    // produced the conversion data that will arrive at the next DRDY.
    ...
    // channel_sel is latched at EOC (pre-toggle adc_mux_sel). Because mux_sel
    // has already toggled by DRDY time, channel_sel is the only correct way
    // to identify which channel's data just arrived.
    always @(posedge clk_100m or negedge rst_n) begin
        if (!rst_n) begin
            ch1_data_reg <= 12'd0;
            ch2_data_reg <= 12'd0;
