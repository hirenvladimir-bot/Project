...
//
// Button assignment (matches pocketscope_sim2_0 PBs):
//   PB0 = amplitude up   (+10, hold for auto-repeat ~10Hz after 500ms)
//   PB1 = amplitude down (-10, hold for auto-repeat ~10Hz after 500ms)
//   PB2 = mod depth up   (+16, hold for auto-repeat ~10Hz after 500ms)
//   PB3 = mod depth down (-16, hold for auto-repeat ~10Hz after 500ms)
//   PB4 = scope trigger level (+10, wrap; hold for auto-repeat)
//=============================================================================
...
(
    ...
    //=========================================================================
    // Auto-repeat — hold a button for continuous adjustment
    //
    // Matches the user manual: "按住 PB 可持续调节" (hold for continuous).
    // Single press  → immediate edge action (one step).
    // Hold > 500ms  → auto-repeat at ~10Hz (one step every ~100ms).
    //
    // A shared 22-bit timer generates a 100ms tick; per-button 3-bit hold
    // counters track the initial-delay phase.  When a button is released the
    // counter resets to zero, so a new press always starts with the initial
    // delay (avoids accidental rapid-fire on a brief release).
    //=========================================================================
    localparam REPEAT_DIV = 2500000;   // 100ms at 25MHz (22 bits)
    localparam HOLD_INIT  = 5;         // 500ms initial delay (5 × 100ms)
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            repeat_cnt <= 0;
        else if (repeat_cnt == REPEAT_DIV - 1)
            repeat_cnt <= 0;
        else
            repeat_cnt <= repeat_cnt + 1'b1;
    end
    assign repeat_tick = (repeat_cnt == REPEAT_DIV - 1);

    // Per-button hold counters and combined action strobe
    reg  [2:0] hold_cnt   [0:4];
    wire       btn_action [0:4];   // fires on press edge + auto-repeat ticks

    genvar ai;
    generate for (ai = 0; ai < 5; ai = ai + 1) begin : autorepeat_block
        always @(posedge clk or negedge rst_n) begin
            if (!rst_n)
                hold_cnt[ai] <= 0;
            else if (!btn_stable[ai])
                hold_cnt[ai] <= 0;
            else if (repeat_tick && hold_cnt[ai] < 7)
                hold_cnt[ai] <= hold_cnt[ai] + 1'b1;
        end

        // Fire on initial press edge, then on every repeat_tick after the
        // initial hold-off period expires.
        assign btn_action[ai] = btn_rise[ai] ||
            (btn_stable[ai] && repeat_tick && hold_cnt[ai] >= HOLD_INIT);
    end endgenerate
    ...
    //
    // Button actions use btn_action[] (edge + auto-repeat) instead of raw
    // btn_rise[], so holding a button produces continuous adjustment at
    // ~10Hz after a 500ms initial delay.
    //=========================================================================
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            device_mode         <= 2'b00;
            ...
            // ---- Button actions (edge + auto-repeat, one step per strobe) ----
            // PB0: amplitude +10  (clamped to 255 max)
            if (btn_action[0] && amplitude <= 8'd245)
                amplitude <= amplitude + 8'd10;
            else if (btn_action[0])
                amplitude <= 8'd255;

            // PB1: amplitude -10  (clamped to 0 min)
            if (btn_action[1] && amplitude >= 8'd10)
                amplitude <= amplitude - 8'd10;
            else if (btn_action[1])
                amplitude <= 8'd0;

            // PB2: mod depth +16
            if (btn_action[2] && mod_depth <= 8'd239)
                mod_depth <= mod_depth + 8'd16;
            else if (btn_action[2])
                mod_depth <= 8'd255;

            // PB3: mod depth -16
            if (btn_action[3] && mod_depth >= 8'd16)
                mod_depth <= mod_depth - 8'd16;
            else if (btn_action[3])
                mod_depth <= 8'd0;

            // PB4: scope trigger level +10 (wraps around)
            if (btn_action[4]) begin
